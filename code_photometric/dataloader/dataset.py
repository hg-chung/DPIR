...
class MemDataset(object):
    def __init__(self,pts,masks,pose,image,K):
        self.pts = pts
        self.pose = pose
        self.masks = masks
        self.image = image
...
class Data():
    def __init__(self, args):
        ...
        K = None
        images, poses, hwf, i_split = load_blender_data(self.datadir, args.half_res, args.testskip)
        masks = images[..., -1:]
        images = images[..., :3]
        H, W, focal = hwf
        H, W = int(H), int(W)
    ...
    def genpc(self):
        ...
        item = MemDataset(pts,self.masks,self.poses, self.images,self.K)
        return item
