...
class MemDataset(object):
    def __init__(self,masks,pose,image,normal,K):
        self.pose = pose
        self.masks = masks
        self.image = image
        self.K = K
...
class Eval_Data():
    def __init__(self, args):
        ...
        K = None
        images, poses, all_normals, hwf, i_split = load_blender_data(self.datadir, args.half_res, args.testskip)
        masks = images[..., -1:]
        images = images[..., :3]
        H, W, focal = hwf
        H, W = int(H), int(W)
    ...
    def genpc(self):
        [H, W, focal] = self.hwf
        item = MemDataset(self.masks,self.poses, self.images,self.normals,self.K)
        return item
