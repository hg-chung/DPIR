...
def load_mem_data(mem):
    ...
    R, T2 = R, -(T1[: ,None ,:] @ R)[: ,0]
    return mem.pts, mem.image, mem.K, R, T1, T2, poses, mem.masks

def load_test_data(mem):
    poses = mem.pose # w2c
    R, T1 = (poses[:, :3, :3]), poses[:, :3, -1]
    R, T2 = R, -(T1[: ,None ,:] @ R)[: ,0]
    return mem.image, mem.K, R, T1, T2, poses, mem.masks, mem.normals, mem.ball_mask, mem.ball_normal
...
def remove_outlier(pts):
    ...
    cl, ind = pcd.remove_statistical_outlier(nb_neighbors=10, std_ratio=10.0)
