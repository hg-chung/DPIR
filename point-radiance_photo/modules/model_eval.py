...
class CoreModel(torch.nn.Module):
    def __init__(self, args,n_points):
        ...
        imagesgt, K, R, T1, T2, poses, masks, normals,ball_mask, ball_normal = load_test_data(args.memitem)
        ...
        self.ball_mask = torch.tensor(ball_mask, dtype=torch.float32)
        self.ball_normal = torch.tensor(ball_normal, dtype=torch.float32)
        self.valid_idx = torch.where(self.ball_mask > 0.5)
        self.valid_ball_normal = self.ball_normal[self.valid_idx]
    ...
    def compute_normals_and_feature_vectors(self):
        ...
        return normals,feature_vectors# normals and feature vectors for albedo
    ...
    def forward(self, id, vis_train =False):
        ...
        features = torch.cat([features, normals, albedo_features, spec_features, vis, depth.reshape(-1,1)], dim=-1)
        ...
        base, normal_img, albedo, spec_ref, vis, depth = feat[..., :3], feat[..., 3:6], feat[..., 6:9], feat[..., 9:12], feat[..., 12:13],feat[..., 13:]
        base = feat[...,:3]
        ...
        image = base
        return image.clamp(min=0, max=1), normal_img, albedo, spec_ref, vis, depth, self._output
