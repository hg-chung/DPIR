...
from dataloader.load_diligent_eval import load_diligent_eval
...
class MemDataset(object):
    def __init__(self,pose,image,mask,K,light_direction,light_intensity,normal):
        self.pose = pose
        self.mask = mask
        self.image = image
        self.K = K
...
class Eval_Data():
    def __init__(self, args):
        ...
        images, masks, poses, light_dir, light_int, normal, hwf, K, i_split = load_diligent_eval(self.datadir)     
    ...
    def genpc(self):
        item = MemDataset(self.poses,self.images,self.masks,self.K, self.light_direction, self.light_intensity,self.normal)
        return item
