...
from modules.utils import device, mse2psnr, SSIM, LPIPS, \
    grad_loss, safe_path, set_seed
from modules.image_losses import PyramidL2Loss, ssim_loss_fn
...
class Trainder(object):
    def __init__(self, args):
        ...
        if self.dataname == 'bear':
            args.refine_n = 2
        self.lpips = LPIPS()
        self.lr1, self.lr2  = args.lr1, args.lr2
        self.lrexp = args.lrexp
    ...
    def set_radius(self):
        self.model.raster_settings = PointsRasterizationSettings( 
            bin_size=25, 
            image_size=self.model.img_s, 
            radius=self.model.point_radius, 
            points_per_pixel=self.model.raster_n, 
        )
    ...
    def set_optimizer(self, lr1=5e-4, lr2=5e-4): # set optimizer
        ...
        model_params = list(map(lambda x: x[1], list(filter(lambda kv: kv[0] in model_list,
                                self.model.named_parameters())))) # network
        other_params = list(map(lambda x: x[1], list(filter(lambda kv: kv[0] not in model_list,
                                self.model.named_parameters())))) # point
        optimizer = torch.optim.Adam([
            {'params': other_params, 'lr': lr1}, # point
            {'params': model_params, 'lr': lr2} # network
            ])
        
        lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, self.lrexp, -1)
        self.optimizer, self.lr_scheduler = optimizer, lr_scheduler

    def train(self,epoch_n=40, shadow_train = False):
        ...
        for epoch in range(epoch_n):
            ...
            for id in tqdm(ids): 
                self.set_radius()     
                images, normal_img, albedo_img, spec_img, pred_mask, shadow, output = self.model(id,shadow_train) 
                ...
                if epoch == epoch_n-1: # visual
                    ...
                    vis_img = shadow.expand(normal_img.shape).detach().cpu().data.numpy().squeeze()
